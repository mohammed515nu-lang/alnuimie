...
// JWT_SECRET checked in middleware/auth.js - reuse from there
...
/** تغيير كلمة المرور للمستخدم المسجّل (يتطلب كلمة المرور الحالية) */
  ...
  try {
    ...
    if (!user.password) {
      return res.status(400).json({
        error: 'هذا الحساب مسجّل عبر Google. استخدم تسجيل الدخول بجوجل أو استعادة عبر البريد إن وُجدت كلمة مرور.',
      });
    }
    ...
    res.status(500).json({ error: 'تعذر تغيير كلمة المرور', message: error.message });
  }
});
...
// Google OAuth - Debug endpoint to check configuration
router.get('/google/debug', (req, res) => {
  ...
  res.json({
    ...
    instructions: [
      '1. Copy the exactRedirectUri value above',
      '2. Go to Google Cloud Console → APIs & Services → Credentials',
      '3. Click on your OAuth 2.0 Client ID',
      '4. In "Authorized redirect URIs", add the exactRedirectUri value',
      '5. Click Save',
