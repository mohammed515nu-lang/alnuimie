...
function projectPublic(u) {
  ...
  return {
    ...
    website: u.website,
    lastSeenAt: u.lastActiveAt ? new Date(u.lastActiveAt).toISOString() : undefined,
  };
}
...
// POST /api/users/me/presence — تحديث وقت آخر نشاط (للظهور كـ «آخر ظهور»)
router.post('/me/presence', authenticate, async (req, res) => {
  try {
    const now = new Date();
    await User.findByIdAndUpdate(req.userId, { lastActiveAt: now });
