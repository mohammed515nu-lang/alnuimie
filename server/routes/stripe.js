...
router.post('/create-payment-intent', async (req, res) => {
  try {
    ...
    const { amount, supplier, project, purchase, currency = 'usd', idempotencyKey } = req.body;
    ...
    const safeIdempotencyKey =
      String(idempotencyKey || req.headers['x-idempotency-key'] || '').trim() || `payment-${req.userId}-${Date.now()}`;
...
router.get('/payment-status/:paymentId', async (req, res) => {
  try {
    ...
    // إذا كانت دفعة Stripe، نجلب آخر حالة من Stripe مباشرة
