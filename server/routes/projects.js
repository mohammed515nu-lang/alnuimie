...
router.post('/', async (req, res) => {
  try {
    ...
    if (req.user && req.userRole === 'client') {
      ...
      req.body.createdBy = req.userId;
      const contractorField = req.body.contractor;
      if (!contractorField) {
        return res.status(400).json({ error: 'يجب ربط المشروع بمقاول (معرّف المقاول)' });
      }
      const contractorId =
        typeof contractorField === 'object' && contractorField !== null && contractorField._id
          ? String(contractorField._id)
          : String(contractorField);
      if (!mongoose.Types.ObjectId.isValid(contractorId)) {
        return res.status(400).json({ error: 'معرّف المقاول غير صالح' });
      }
      const contractorUser = await User.findById(contractorId).select('role');
      if (!contractorUser || contractorUser.role !== 'contractor') {
        return res.status(400).json({ error: 'المستخدم المحدد ليس مقاولًا' });
      }
