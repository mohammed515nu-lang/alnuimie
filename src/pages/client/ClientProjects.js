...
import { projectsAPI, getUser } from "../../utils/api";
...
export default function ClientProjects() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      try {
        ...
        // Add itemType for consistency if missing
        const formattedProjects = projectsWithContractor.map(p => ({
          ...p,
          itemType: 'project'
        }));
  ...
  const myProjects = projects.filter(p => {
    const client = typeof p.client === 'object' ? (p.client._id || p.client.id) : p.client;
    return client === userId;
  });

  const displayPool = activeTab === 'mine' ? myProjects : projects;

  const filteredProjects = selectedCategory === 'الكل'
    ? displayPool
    : displayPool.filter(p => p.category === selectedCategory);
  ...
  return (
    <div style={{
      ...
      <div style={{
        position: 'fixed',
        inset: 0,
        background: 'radial-gradient(circle at 20% 30%, rgba(58, 66, 79, 0.1) 0%, transparent 65%)',
        pointerEvents: 'none',
        zIndex: 0
      }} />
      <div style={{
        position: 'fixed',
        inset: 0,
        background: 'radial-gradient(circle at 80% 70%, rgba(74, 85, 104, 0.08) 0%, transparent 65%)',
        pointerEvents: 'none',
        zIndex: 0
      }} />
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ marginBottom: 30 }}>
          <h2 style={{
            ...
            margin: '0 0 12px 0',
            letterSpacing: '-1.2px',
            textShadow: '0 2px 10px rgba(58, 66, 79, 0.1)'
          }}>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 12,
              border: 0,
              background: activeTab === 'mine' ? BRAND.gradient : 'transparent',
              color: activeTab === 'mine' ? '#fff' : BRAND.text,
              fontWeight: 700,
              cursor: 'pointer',
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 12,
              border: 0,
              background: activeTab === 'all' ? BRAND.gradient : 'transparent',
              color: activeTab === 'all' ? '#fff' : BRAND.text,
              fontWeight: 700,
              cursor: 'pointer',
              transition: 'all 0.3s ease'
            }}
          >
            استكشاف المشاريع ({projects.length})
          </button>
        </div>
          ...
          {categories.map(cat => (
            <button
              ...
              style={{
                ...
                color: selectedCategory === cat ? '#fff' : BRAND.text,
                ...
                cursor: 'pointer',
                transition: 'all 0.3s ease',
                boxShadow: selectedCategory === cat ? BRAND.shadows.accent : BRAND.shadows.sm
              }}
              onMouseOver={e => {
            ...
            {filteredProjects.map(p => (
              <div
                ...
                style={{
                  ...
                  overflow: 'hidden',
                  border: `2px solid ${BRAND.border || 'rgba(0,0,0,0.05)'}`,
                ...
                onMouseOver={e => {
                  ...
                  e.currentTarget.style.borderColor = BRAND.accentAlpha[30];
                }}
                onMouseOut={e => {
                  e.currentTarget.style.transform = 'none';
                  e.currentTarget.style.boxShadow = BRAND.shadows.md;
                  e.currentTarget.style.borderColor = BRAND.accentAlpha[15];
                }}
              >
                {/* Image */}
                <div style={{
